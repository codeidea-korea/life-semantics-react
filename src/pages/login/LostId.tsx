...
const IndexPage = () => {
    ...
    const handlePhoneNumber = (event: React.ChangeEvent<HTMLInputElement>) => {
		...
		let reducePhoneNumber = value.replace(/[^0-9]/g, '').replace(/^(\d{0,3})(\d{0,4})(\d{0,4})$/g, "$1-$2-$3").replace(/(\-{1,2})$/g, "");
		setUserInfoForTempPass({...userInfoForTempPass, find_phone:reducePhoneNumber});
    };

    const handleName = (event: React.ChangeEvent<HTMLInputElement>) => {
        const {value} = event.currentTarget;
		let reduceUserName = value.replace(/[^ㄱ-ㅎ가-힣|a-zA-Z]/g, '').replace(/^(\d{0,3})(\d{0,4})(\d{0,4})$/g, "$1-$2-$3").replace(/(\-{1,2})$/g, "");
		setUserInfoForTempPass({...userInfoForTempPass, find_name:reduceUserName});
    };
	...
	return (
		...
		<div className="Login">
			...
			<div className="LostId">
				...
				<input id="find_name" type="text" placeholder="이름 입력" value={userInfoForTempPass.find_name} onChange={handleName}/>
				<input id="find_phone" type="text" placeholder="전화번호(숫자만 입력)" value={userInfoForTempPass.find_phone} maxLength={13}  pattern="\d*" onChange={handlePhoneNumber}/>
				{alertState}
				</div>
			</div>
			</div>
