...
const LostPassword = () => {
	...
	const handlePhoneNumber = (event: React.ChangeEvent<HTMLInputElement>) => {
		...
		if (name === 'userPhoneTemp') {
		setUserInfoForTempPass({
			...userInfoForTempPass,
			userPhoneTemp: reducePhoneNumber
		})
		}
		else {
	...
	const handleUserInfoForTempPass = (event: React.ChangeEvent<HTMLInputElement>) => {
		...
		let reduceUsername = value.replace(/[^ㄱ-ㅎ가-힣|a-zA-Z]/g, '').replace(/^(\d{0,3})(\d{0,4})(\d{0,4})$/g, "$1-$2-$3").replace(/(\-{1,2})$/g, "");
		setUserInfoForTempPass({...userInfoForTempPass, userName:reduceUsername});
	}
	
	const requestTempPass = () => {
		api
		.post(`/users/temporary-password?userName=${userInfoForTempPass.userName}&userPhone=${userInfoForTempPass.userPhoneTemp}`, null)
		.then((res) => {
			console.log(res)
	...
	const handleUserInfoForNewPass = (event: React.ChangeEvent<HTMLInputElement>) => {
		...
		else if (value !== userInfoForNewPass.userNewPass) {
			setAlertText({
			...alertText,
	...
	return (
		...
		<div className="Login">
			...
			<p>임시 비밀번호 발급</p>
			<label>이름</label>
			<input type="text" placeholder="이름 입력" maxLength={10} name="userName" value={userInfoForTempPass.userName} onChange={handleUserInfoForTempPass}/>
			<label>휴대폰 번호</label>
			<input type="text" placeholder="숫자만 입력" name="userPhoneTemp" value={userInfoForTempPass.userPhoneTemp} onChange={handlePhoneNumber} maxLength={13}/>
